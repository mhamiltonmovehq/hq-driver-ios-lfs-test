...
@objc class TokenWrapper: NSObject {
    ...
    @objc func verifyToken(jwt: String, caller: TokenWrapperProtocol) {
        ...
        let appName = "Driver"
        do {
            try TokenService.shared.verifyToken(jwt: jwt, productName: appName, completion: verifyCompleted)
        }
        catch let error {
            print(error)
        }
    ...
    @objc func refreshToken(jwt: String) {
        TokenService.shared.refreshToken(jwt: jwt, productName: "Driver", completion: verifyRefreshCompleted)
    }
    ...
    @objc func jsonToTokenDecoder(data: Data) -> HubToken?
    {
        let hubToken: HubToken
        let decoder = JSONDecoder()
        do {
            hubToken = try decoder.decode(HubToken.self, from: data)
        }
        catch let error {
            print(error)
            return nil
...
class TokenService {
    ...
    func getToken (info: SessionInfo, completion: @escaping (RestResult<HubToken>) -> Void) throws {
        let del = UIApplication.shared.delegate as? SurveyAppDelegate
        let request = RestSyncRequest()
        var hubToken: HubToken? = nil
        let headerVals = ["Accept" : "application/json"]
            ...
            try request.executeHttpRequest(httpMethod: "POST", queryParameters:nil, bodyData:body , headerData: headerVals) {result in
                ...
                case .Success(let response):
                    ...
                    do {
                        ...
                        KeyChainAdapter.setDataValue(value: jsonData, key: "hubToken")
                    } catch let error {
                        print(error)
    ...
    func refreshToken (jwt: String, productName: String, completion: @escaping (RestResult<HubToken>) -> Void) {
    ...
    func verifyToken (jwt: String, productName: String, completion: @escaping (RestResult<Bool>) -> Void) throws {
        let headerValues = ["Authorization" : "Bearer " + jwt]
        let request = RestSyncRequest()
        let params = ["product" : productName]
