...
class HubActivationService {
    ...
    func activate (username: String, password: String, udid: String, appName: String, deviceType: String,
        ...
        do {
            try tokenService.getToken(info: sInfo){ result in
                switch (result) {
                case .Error(let message):
                    completion(.Error(message))
                case .Success(_):
                    do {
                        try self.getHubActivationRecord(jwt: (self.del?.session.access_token)!, info: sInfo) { result in
                            switch (result) {
                                case .Error(let message):
                                    completion(.Error(message))
    ...
    func getHubActivationRecord (jwt: String, info: SessionInfo, completion: @escaping (RestResult<HubActivationRecord>) -> Void) throws {
